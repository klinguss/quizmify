...
if (type==='open_ended'){
    questions= await strict_output(
        'You are a helpful AI that is able to generate a pair of questions and answers, the length of the answer should not exceed 15 words, store all the pairs of answers and questions in JSON array',
        new Array(amount).fill(
        `You are to generate a random hard open-ended question about ${topic}`,
        ),
        {
            question:"question",
            answer: "answer with max lengths of 15 words",
        }
    );
} else if (type==="mcq"){
    questions = await strict_output(
        ...
        new Array(amount).fill(
            `You are to generte a random mcq question about ${topic}`
        ),
        {
            question: 'quesions',
